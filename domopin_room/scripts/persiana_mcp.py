...
IODIRB = 0x01           # Byte B  0 salida 1 entrada
OLATA = 0x14            # Salidas A
OLATB = 0x15            # Salidas B
GPIOA = 0x12            # Registos para entradas A
GPIOB = 0x13            # Registro entras B
...
def Salidas():
	...
	if (PERSIANA[0]==0 and ANT_PERSIANA[0]==1):	
		#PinRSP.off()
                bus.write_byte_data(DEVICE,OLATB,bus.read_byte_data(DEVICE,OLATB)& 0b01111111)
		EscribirFichero()
	
	if (PERSIANA[0]==1 and ANT_PERSIANA[0]==0):
		#PinRSP.on()
		bus.write_byte_data(DEVICE,OLATB,bus.read_byte_data(DEVICE,OLATB)| 0b10000000)
	
	if (PERSIANA[1]==0 and ANT_PERSIANA[1]==1):
		#PinRBP.off()
                bus.write_byte_data(DEVICE,OLATB,bus.read_byte_data(DEVICE,OLATB)& 0b10111111)
		EscribirFichero()
	
	if (PERSIANA[1]==1 and ANT_PERSIANA[1]==0):
		#PinRBP.on()
                bus.write_byte_data(DEVICE,OLATB,bus.read_byte_data(DEVICE,OLATB)| 0b01000000)
...
##	time.sleep(2)
	...
	if (P[1]==1 and POS_DESEADA_WEB<POS_ACTUAL and SENSORES[1]==1):
                P[7]=1
                P[1]=0
